...
<script type="text/javascript">
	...
	function formCheck(){
		if(!checkId(joinForm.id.value)) {
			return false;
		}else if(!checkName(joinForm.name.value)){
			return false;	
		}else if(!checkPw(joinForm.pw.value, joinForm.pw2.value)){
			return false;
		}else if(!checkEmail(joinForm.email.value)){
			return false;
		}else if(!checkPhone(joinForm.phone.value)){
			return false;
		}
		return true;
	}
    ...
    function checkId(id){
    	...
    	/*//아이디 유효성검사
 		...
 		if(!idRegExp.test(id)){
 			alert("아이디는 대소문자 숫자 4~12글자로 입력해야합니다.");
 			joinForm.id.value="";
 			joinForm.id.focus();
 			return false;
 		} 	   		
    ...
    function checkPw(pw, pw2) {
    	if(!checkExistData(pw, "비밀번호를") && !checkExistData(pw2, "비밀번호 확인을")){
    		return false;
    	}
            ...
            <table>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td><input type="password" name="pw2" placeholder="비밀번호를 확인하세요"></td>
                </tr>
                <tr class="email">
                  <th><span>이메일</span></th>
                  <td>
                    <input type="text"  name="email" placeholder="">
                    <a href="javascript:;" class="btn btn-primary">인증번호 발송</a>
                  </td>
                </tr>
                <tr>
